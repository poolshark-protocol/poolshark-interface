  ...
  return (
    ...
    <Link href="/pool/view">
      ...
      >
        ...
        onClick={() => {navigator.clipboard.writeText(tokenOneAddress)}}>{tokenOneName} <span>{tokenOneDisplay}</span></h1>
        <h1 className="text-xs mt-2"
        onClick={() => {navigator.clipboard.writeText(tokenZeroAddress)}}>{tokenZeroName} <span>{tokenZeroDisplay}</span></h1>
        </div>
       
          <h1 className="text-xs"
          onClick={() => {navigator.clipboard.writeText(poolAddress)}}
          >Pool: <span>{poolDisplay}</span></h1>
        </div>
