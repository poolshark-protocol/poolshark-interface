              ...
              <Dialog.Panel className="w-full max-w-sm transform overflow-hidden rounded-xl bg-black text-white border border-grey2 text-left align-middle shadow-xl px-5 py-5 transition-all">
                ...
                <div className="mt-4 space-y-1">
                  ...
                  onClick={() => switchNetwork(5)}
                   className={`${ chainId === 5 ? ' bg-background' :'hover:bg-[#0C0C0C] hover:border-[#1C1C1C]'} flex justify-between items-center w-full p-2 rounded-xl  border border-black cursor-pointer`}>
                    <div className="flex gap-x-2 items-center">
                        <img src="/static/images/eth_icon.svg" />
                        Goerli
                    </div>
                    <div className={`${ chainId === 5 ? ' flex gap-x-2 items-center text-main text-xs' :'hidden'}`}>
                        Connected
                        <div className="h-1.5 w-1.5 bg-green-500 rounded-full"/>
                    </div>
                  </div>
