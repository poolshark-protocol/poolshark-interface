...
describe('TickMath tests', () => {
  ...
  test('price string of 3000.00 converts to tick 80067', () => {
    expect(TickMath.getTickAtPriceString('3000.000000', tokenA, tokenB)).toBe(80067);
  });
  ...
  test('sqrt price for 1.00 converts to price string', () => {
    ...
    const priceString1 = TickMath.getPriceStringAtSqrtPrice(sqrtPrice1, tokenA, tokenB)
    // check priceString
    expect(priceString1).toStrictEqual('1')
    // check sqrtPrice at priceString
    expect(TickMath.getSqrtPriceAtPriceString(priceString1, tokenA, tokenB)).toStrictEqual(sqrtPrice1);
  });
  ...
  test('price string for 3.402e+38 converts to tick', () => {
    ...
    const priceString = TickMath.getPriceStringAtSqrtPrice(priceString2, tokenA, tokenB)
  ...
  test('position of tick 0 to 100 zeroForOne true gives expected price', () => {
    const lowerTick = 0
    const upperTick = 100
    const price = getAveragePrice(tokenA, tokenB, lowerTick, upperTick, true, BigNumber.from('20051041647900280328782'), parseUnits('100', 18))
    // check price
  ...
  test('position of tick 0 to 100 zeroForOne false gives expected price', () => {
    const lowerTick = -100
    const upperTick = 0
    const price = getAveragePrice(tokenA, tokenB, lowerTick, upperTick, false, BigNumber.from('20051041647900280328782'), parseUnits('100', 18))
    // check price
