...
export const formatOFin = (numString: string, fixed: number): string => {
  ...
  if (parseFloat(numString) < 1)
    return `${1 * parseFloat(parseFloat(numString).toFixed(fixed))}`;
...
export function inputHandler(
  ...
  if (result == "") {
    ...
    return [result, parseUnits(result, token.decimals)];
  }
}

export function getFeeApy(rangePool: any): string {
  if (!rangePool.tvlUsd || rangePool.tvlUsd == 0) return "0.00";
  return ((((rangePool.feesUsd ?? 0) * 365) / rangePool.tvlUsd) * 100).toFixed(
    2,
  );
}
...
export function parseUnits(value: string, decimals: number): BigNumber {
  ...
  if (
    ...
    floatValue.toString().indexOf("e-") != -1
  ) {
    // example: 1.36e-7
    const decimalCount = floatValue
      .toString()
      .split("e-")[0]
      .split(".")[1].length;
