...
export default function Sale() {
  ...
  const wethToken: baseToken = {
    address: saleConfig.wethAddress as `0x${string}`,
    decimals: 18,
  };
  const finToken: baseToken = {
    address: saleConfig.finAddress as `0x${string}`,
    decimals: 18,
  };
  ...
  useEffect(() => {
    ...
    const startPrice = parseFloat(
      TickMath.getPriceStringAtSqrtPrice(startSqrtPrice, wethToken, finToken),
    );
    setStartUsdPrice(`${(ethUsdPrice / startPrice).toFixed(2)}`);
    const endPrice = parseFloat(
      TickMath.getPriceStringAtSqrtPrice(endSqrtPrice, wethToken, finToken),
    );
  ...
  const { data: filledAmount } = useContractRead({
    ...
    chainId: chainId,
    watch: true,
    enabled: chainId == saleConfig.chainId,
    onSuccess(data) {
  ...
  useEffect(() => {
    (async () => {
      if (
        isNaN(parseFloat(ethReceived)) ||
        parseFloat(startUsdPrice) == 0 ||
        parseFloat(endUsdPrice) == 0
      )
        return;
      const liquidity = JSBI.BigInt(saleConfig.limitLP.liquidity);
      const ethAmount = JSBI.BigInt(parseUnits(ethReceived, 18));
      const currentSqrtPrice = TickMath.getNewSqrtPrice(
        startSqrtPrice,
        liquidity,
        ethAmount,
        true,
      ...
      const percentFill =
        (parseFloat(currentUsdPrice) - parseFloat(startUsdPrice)) /
        (parseFloat(endUsdPrice) - parseFloat(startUsdPrice));
  ...
  const handleClick = async () => {
    if (chainId == saleConfig.chainId) {
      router.push(
        `/?chain=${chainProperties["fin-token"]["sale"]["chainId"]}&from=${chainProperties["fin-token"]["sale"]["wethAddress"]}&fromSymbol=ETH&to=${chainProperties["fin-token"]["sale"]["finAddress"]}&toSymbol=FIN`,
      );
    } else {
      ...
      <div className="flex flex-col pt-10 pb-32 md:pb-0 text-white relative min-h-[calc(100vh-76px)] container mx-auto md:px-0 px-3">
        <div className="flex md:flex-row flex-col justify-between w-full items-start md:items-center gap-y-5">
          <div className="flex items-center gap-x-4">
            ...
            <div className="flex flex-col gap-y-2">
              <div className="flex text-lg items-center text-white">
                <h1>$FIN SALE</h1>
                <a
                  href={`${chainProperties["fin-token"]["sale"]["explorerUrl"]}`}
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-x-3 text-grey1 group cursor-pointer"
                >
                  <span className="-mb-1 text-light text-xs ml-8 group-hover:underline"></span>{" "}
                  <ExternalLinkIcon />
                </a>
              </div>
              <div className="flex text-xs text-[#999999] items-center gap-x-3">
                ...
                {isLoading ? (
                  ...
                  <span className="bg-grey/50 rounded-[4px] text-grey1 text-xs px-3 py-0.5">
                    {(
                      (parseFloat(ethReceived) / parseFloat(ethTotal)) *
                      100
                    ).toFixed(2)}
                    % FILLED
                  </span>
          ...
          <div className="flex items-center gap-x-4 w-full md:w-auto">
            {networkName == chainIdsToNames[saleConfig.chainId] ? (
              <Link
                href={`/?chain=${chainProperties["fin-token"]["sale"]["chainId"]}&from=${chainProperties["fin-token"]["sale"]["wethAddress"]}&fromSymbol=ETH&to=${chainProperties["fin-token"]["sale"]["finAddress"]}&toSymbol=FIN`}
              >
                <button className="bg-main1 hover:opacity-80 transition-all border whitespace-nowrap w-full rounded-full text-center border-main transition-all py-2.5 px-20 text-sm uppercase cursor-pointer text-[13px] text-main2">
                  {"BUY FIN"}
                </button>
              </Link>
            ) : (
              ...
              >
                {"SWITCH TO " +
                  chainProperties[saleConfig.networkName]["chainName"]}
              </button>
            )}
          </div>
        </div>
        <div className=" w-full mt-8 gap-10">
          <div className="border h-min border-grey rounded-[4px] w-full p-5 pb-7">
            ...
            <div className="flex md:flex-row flex-col gap-6 mt-2">
              <div className="border w-full border-main rounded-[4px] flex flex-col w-full items-center justify-center gap-y-4 h-32 bg-main1 ">
                <span className="text-main2/60 text-[13px]">CURRENT PRICE</span>
                {isLoading ? (
                ...
                {isLoading ? (
                  ...
                  <span className="text-white text-center xl:text-4xl md:text-3xl text-2xl">
                    ...
                    {new Intl.NumberFormat("en-US").format(
                      parseFloat(
                        (parseFloat(ethReceived) * ethUsdPrice).toFixed(2),
                      ),
                    )}
                    <span className="text-grey2 md:text-base text-sm">
                      {" "}
                      / $
                      {new Intl.NumberFormat("en-US").format(
                        parseFloat(
                          (parseFloat(ethTotal) * ethUsdPrice).toFixed(2),
                        ),
                      )}
                    </span>
                  </span>
          ...
          <div className="mt-10  grid relative bg-dark border border-grey md:h-[350px] flex items-end">
            ...
            <div
              className="svg-container bottom-0"
            ...
            >
              ...
              <div
                className="absolute w-full h-full bottom-0 left-0 p-5 animatedFill"
